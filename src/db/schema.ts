 ...
 */
export const starterSchema = pgSchema("cravings");
 ...
 *
 * Indexes:
 * - `cravings_histories_user_idx` on `user_id` for efficient user-scoped queries
 */
export const histories = starterSchema.table(
  ...
  table => ({
    userIdx: index("cravings_histories_user_idx").on(table.user_id),
  })
);
