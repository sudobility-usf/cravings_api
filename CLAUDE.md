...
Backend API server for the Cravings application.

**Package**: `cravings_api` (private, not published)
...
- All API responses must be wrapped in `BaseResponse<T>` using `successResponse()` or `errorResponse()` from `@sudobility/cravings_types`
