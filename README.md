...
Backend API server for the Cravings application. Built with Hono, PostgreSQL (Drizzle ORM), and Firebase Auth.
...
Schema: `cravings`. Tables (`users`, `histories`) are created via raw SQL on startup -- no migration files.
...
- **cravings_client** -- API client SDK that consumes these endpoints
- **cravings_lib** -- Business logic library
